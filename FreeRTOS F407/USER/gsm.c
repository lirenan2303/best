...
const static WGMessageHandlerMap GPRS_MessageMaps[] =  //¶þÎ»Êý×éµÄ³õÊ¼»¯
{
	...
	{UNITRUNBACK,       HandleUnitRunBack},      /*0x0A; µ¥µÆ×ÔÖ÷ÔËÐÐ*/
  ...
  {ELECVERSION,       HandleElecVersQuery},    /*0x0E; ²éµçÁ¿°åÈí¼þ°æ±¾ºÅ*/	
	...
	{SENDELEC,          HandleSend},             /*0x88; Íø¹ØÊý¾Ý²éÑ¯*/
	{SENDELECSOFT,      HandleSend},             /*0x8E; ²éµçÁ¿²É¼¯Èí¼þ°æ±¾ºÅ*/
	{SENDELEFTPUP,      HandleSend},             /*0x9E; µçÁ¿²É¼¯Ä£¿éÔ¶³ÌÉý¼¶*/
  {SENDELERESET,      HandleSend},             /*0x3F; µçÁ¿²É¼¯Ä£¿é¸´Î»*/	
...
static void vGSMTask(void *parameter)
{
  ...
  portTickType CSQ_TCP_lastT=0;
	...
	for(;;)
	{
		...
		if(uxQueueMessagesWaiting(GSM_GPRS_queue) <= 1)//·¢ËÍÍø¹ØÊý¾Ý
		{
			WG_DataSample(0xFF);
		}
...
void GSMInit(void)
{
  GSMInitHardware();
	...
	/*************************************************************************************************
	 ÓÉÓÚGSMÄ£¿é°üº¬Í¸´«ºÍ·ÇÍ¸´«Ä£Ê½£¬¶øÇÒÔÚ¹¤×÷¹ý³ÌÖÐÐèÒªÇÐ»»£¬¹ÊGPRS·¢ËÍ½ÓÊÕ±ØÐëÔÚÒ»¸öÈÎÎñÄÚ
	 *************************************************************************************************/
}
